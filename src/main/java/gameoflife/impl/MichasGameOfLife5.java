...
/**
 * Based on {@link MichasGameOfLife4} but with the following optimizations:<ul>
 *     <li>Multi threaded calculation -- see {@link MichasGameOfLife4a},</li>
 *     <li>Skips the calculation of inner cells when it detects 3 consecutive empty rows -- see {@link MichasGameOfLife4b},</li>
 *     <li>Removed empty tiles -- see {@link MichasGameOfLife4c}</li>
 * </ul>
 */
public class MichasGameOfLife5 implements GameOfLife {
    ...
    private abstract static class TileOfTiles<T extends Tile> implements Tile {
        ...
        public abstract void prepareCalculationOfNextGeneration();
    ...
    private class MegaTile extends TileOfTiles<TileOf32x32Cells> {
        ...
        @Override
        public void prepareCalculationOfNextGeneration() {
            for (int i = 0; i < n; ++i) {
                final Tile tile = nonNullChildren[i];
                final Runnable task = new Runnable() { @Override public void run() {
                    tile.calculateNextGeneration();
                }};
    ...
    private class SuperMegaTile extends TileOfTiles<MegaTile> {
        ...
        public void prepareCalculationOfNextGeneration() {
            for (int i = 0; i < n; ++i) {
                nonNullChildren[i].prepareCalculationOfNextGeneration();
            }
        }
    }
